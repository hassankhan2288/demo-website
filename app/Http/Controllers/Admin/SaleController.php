...
class SaleController extends Controller
{
    ...
    public function adminPosSale($order_id, $user_id, $branch_id)
    {
        $user = Auth::user();
        $orderId = $order_id;
        $userId = $user_id;
        $branchId = $branch_id;
        //dd('ok');
//        $categories = CustomerCategory::where("c_id",$user->user_id)->get();
        $categories = CustomerCategory::where("c_id",$user_id)->get();
        $brands = Brand::where("is_active",1)->get();
//        $products = ProductPricingManagement::select("product_id", "sale_price")->whereHas("product",function($query){
//            $query->where(['is_active'=>true]);
//        })->with(["product"=>function($query){
//            $query->select("id", "name", "code", "image");
//        }])->where("user_id",$user_id)->count();

        $products = ProductPricingManagement::with('product', 'product.brands', 'product.tax','product.discount')->with('product.categories', function ($query) {
            $query->with('discount');
        ...
        return view('admin.partials.pos.pos', compact('products', 'categories', 'brands', 'user', 'orderId', 'userId', 'branchId'));
    }
    public function adminPosSaleSubmitold(Request $request, MSDynamic $api){
...
//        $branch_id = $user->id;
        ...
        if($request->items){
            foreach($request->items as $item){
                ...
                $product_sale['product_id'] = $item['productId'];
                $product_sale['qty'] = $item['qty'];
                $product_sale['net_unit_price'] = $item['price'];
                $product_sale['tax'] = $item['tax'];
                $product_sale['total'] = $item['price']+$item['tax'];
                $product_sale['type'] = 'SINGLE';
                // $product_sale['uom'] = 'SINGLE';
        ...
        // dd($collectData);
        // dispatch(new DynamicsCreateDeliveryOrder($collectData, $user));
    ...
    public function adminPosSaleSubmit(Request $request, MSDynamic $api){
...
//        if ($order_id) {
            // Find the existing sale by $order_id
            $sale = Sale::find($order_id);

            if ($sale) {
                // Update the existing sale
                $sale->update($data);
            } else {
                // Handle the case where the provided $order_id does not match any existing sale
                return response()->json(['error' => 'Invalid order_id provided for update'], 400);
            }
//        } else {
//            // Create a new sale
//            $sale = Sale::create($data);
...
// Delete existing product sales associated with the sale
        Product_Sale::where('sale_id', $order_id)->delete();
        ...
        $price = ProductPricingManagement::where('user_id', $user_id)->whereIn("product_id", $ids)->sum("company_price");
        $sale->product_cost = $price;
        $sale->sale_status = 2;
        $sale->save();

// Insert new product sales
        Product_Sale::insert($sale_data);

// Dispatch the job for DynamicsCreateOrder
        dispatch(new DynamicsCreateOrder($sale_data, $user));
    ...
    public function adminPosProductList(Request $request, $user_id){

        $user = Auth::user();
//        dd($user_id);
//        $products = ProductPricingManagement::select("product_id", "sale_price", "is_favorite", "cate")->whereHas("product",function($query){
//            $query->where(['is_active'=>true]);
//        })->with(["product"=>function($query){
//            $query->select("id", "name", "code", "image", "category_id", "brand_id", 'tax_id', 'tax_method')->with("tax");
//        }])->where("user_id", $user_id)
//            ->paginate(500);

        $products = ProductPricingManagement::select("product_id", "price", 'p_price', "is_favorite", "cate")->whereHas("product",function($query){
            $query->where(['is_active'=>true]);
        })->with(["product"=>function($query){
            $query->select("id", "name", "code", "pack_size", "image", "category_id", "brand_id", 'tax_id', 'tax_method')->with("tax");
        }])->where("user_id", $user_id)
            ->paginate(500);
    ...
    public function adminGetProductByOrder(Request $request){
        $data = [];
        $sale = Sale::find($request->order_id);
//        dd($sale);
        $sale_datas = Product_Sale::where('sale_id', $request->order_id)->get();
        if($sale_datas){
            foreach($sale_datas as $sale_data){
                $image = explode(',', $sale_data->product->image);
                $data[] = [
                    'productId'=>$sale_data->product_id,
                    'image'=>'https://cater-choice-assets.s3.eu-west-2.amazonaws.com/storage/thumbnail/'.$image[0]??"",
                    'name'=>$sale_data->product->name,
                    'price'=>$sale_data->net_unit_price,
                    'option'=>null,
                    'tax'=>$sale_data->tax,
                    'qty'=>$sale_data->qty,
                ];
...
//        dd($data);
    ...
    public function index(Request $request)
    {
            ...
            return view('admin.partials.sale.index',compact('starting_date', 'ending_date', 'warehouse_id', 'sale_status', 'payment_status','company_id'));
    }
    public function posindex(Request $request)
    {
            ...
            return view('admin.partials.sale.pos_index',compact('starting_date', 'ending_date', 'warehouse_id', 'sale_status', 'payment_status','company_id'));
    }
    ...
    public function saleData(Request $request)
    {
        ...
        $q = Sale::where('status', 'website')->whereDate('created_at', '>=' ,$request->input('starting_date'))->whereDate('created_at', '<=' ,$request->input('ending_date'));
        ...
        if($company_id != null)
        //dd($company_id);
            $q = $q->where('user_id', $company_id);
        
        //if($company_id != null)
          //  $q = $q->where('sale_status','>',1);        
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['company'] = $sale->user->name??"";
                $nestedData['branch'] = $sale->branch->name??"";
                ...
                elseif($sale->sale_status == 2){
                    $nestedData['sale_status'] = '<div class="badge badge-warning">Pending</div>';
                    $sale_status = 'Pending';
                }
                else{
                    $nestedData['sale_status'] = '<div class="badge badge-warning">View</div>';
                    $sale_status = 'View';
                }

                if($sale->payment_status == 'pending' || $sale->payment_status == 'Pending')
                    $nestedData['payment_status'] = '<div class="badge badge-danger">'.trans('file.Pending').'</div>';
                elseif($sale->payment_status == 'due' || $sale->payment_status == 'Due')
                    $nestedData['payment_status'] = '<div class="badge badge-danger">'.trans('file.Due').'</div>';
                elseif($sale->payment_status == 'partial' || $sale->payment_status == 'Partial')
                ...
                $nestedData['returned_amount'] = number_format(0, 2);
                $nestedData['paid_amount'] = number_format($sale->paid_amount, 2);
                $nestedData['due'] = number_format($sale->grand_total - $sale->paid_amount, 2);
                $nestedData['pickup_date'] = $sale->pick_date;
                $nestedData['pickup_time'] = $sale->pick_time;
                $nestedData['tax'] = $sale->total_tax;
                $nestedData['status'] = $sale->status;
                $nestedData['pick_time'] = $sale->pick_time;
                $nestedData['pick_date'] = $sale->pick_date;
                $nestedData['ms_order_no'] = $sale->ms_order_no;
                ...
                //                 <li><a href="'.route('admin.sale.invoice', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> '.trans('file.Generate Invoice').'</a></li>
                //                 <li>
                ...
                if($sale->ms_order_no == null){
                    ...
                    <div class="btn-group">
                            <button data-id="'.$sale->id.'" class="pushDynamics btn btn-default btn-sm mr-1" data-loading="<i class=\'fa fa-circle-o-notch fa-spin\'></i>" >Push to Dynamic</button>
                            <button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Status
                              <span class="caret"></span>
                              <span class="sr-only">Toggle Dropdown</span>
                            </button>
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                <li>
                                <a href="'.route('admin.report.invoice',$sale->id).'" class="btn btn-link" target="_blank"><i class="fa fa-eye"></i>View
                                </a>
                                </li>
                                <li>
                                <a href="'.route('admin.sale.status', [$sale->id,2]).'" class="btn btn-link"><i class="fa fa-close"></i>Reject
                                </a>
                                </li>
                                <li>
                                <a href="'.route('admin.sale.status', [$sale->id,1]).'" class="btn btn-link"><i class="fa fa-check"></i>Completed
                                </a>
                                </li>
                                <li><a href="'.route('admin.sale.picklist', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> Picklist </a></li>
                                </ul>
                    </div>';
                ...
                $nestedData['sale'] = array( '[ "'.date(config('date_format'), strtotime($sale->created_at->toDateString())).'"', ' "'.$sale->reference_no.'"', ' "'.$sale_status.'"', ' ""', ' ""', ' ""', ' ""', ' ""', ' ""', ' ""', ' ""', ' ""', ' ""', ' "'.$sale->id.'"', ' "'.$sale->total_tax.'"', ' "'.$sale->total_discount.'"', ' "'.$sale->total_price.'"', ' "'.$sale->order_tax.'"', ' "'.$sale->order_tax_rate.'"', ' "'.$sale->order_discount.'"', ' "'.$sale->shipping_cost.'"', ' "'.$sale->grand_total.'"', ' "'.$sale->paid_amount.'"', ' "'.preg_replace('/[\n\r]/', "<br>", $sale->sale_note).'"', ' "'.preg_replace('/[\n\r]/', "<br>", $sale->staff_note).'"', ' ""', ' ""', ' ""', ' "'.$coupon_code.'"', ' "'.$sale->coupon_discount.'"]'
                );
                //dd($nestedData['date']);
                $data[] = $nestedData;
            }
    ...
    public function posSaleData(Request $request)
    {
        $user = Auth::user();
        $columns = array(
            1 => 'created_at',
        ...
        $q = Sale::where('status', 'POS')->whereDate('created_at', '>=' ,$request->input('starting_date'))->whereDate('created_at', '<=' ,$request->input('ending_date'));
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['pickup_date'] = $sale->pick_date;
                $nestedData['pickup_time'] = $sale->pick_time;
                $nestedData['tax'] = $sale->order_tax;
                $nestedData['status'] = $sale->status;
                $nestedData['pick_time'] = $sale->pick_time;
                $nestedData['pick_date'] = $sale->pick_date;
                $nestedData['ms_order_no'] = $sale->ms_order_no;
                $nestedData['delivery_date'] = $sale->delivery_date ? \Carbon\Carbon::parse($sale->delivery_date)->format('d/m/Y') : '';
                // $nestedData['delivery_date'] = '<div class="input-group date" id="deliveryDate_' . $sale->id . '">
                //                         <input type="text" value="' . $sale->delivery_date . '" data-id="' . $sale->id . '" data-delivery="' . $sale->delivery_date . '" class="form-control delivery-date" />
                ...
                if($sale->ms_order_no == null){
                    ...
                    <div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                ...
                                <li><a href="'.route('admin.sale.picklist', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> Picklist </a></li>
                                 <li>
                                 <button type="button" class="re-order btn btn-link" data-id = "' . $sale->id . '" data-user-id="' . $sale->user_id . '" data-branch-id="' . $sale->branch_id . '">
                                 <i class="fa fa-truck"></i> Re-order</button>
                                </li>
                                </ul>
                    </div>';
...
//        dd($data);
    ...
    {
        $deliveryDate = Sale::find($request->id);
        if ($deliveryDate) {
            $deliveryDate->delivery_date = $request->delivery_date;
            $deliveryDate->save();
            return response()->json($deliveryDate);
        }
        return response()->json(['error' => 'Delivery date update failed'], 500);
    }
    



    public function PicklistData(Request $request)
    {
        //dd($request->all());
        $user = Auth::user();
        $columns = array( 
       ...
       // $q = Sale::whereDate('created_at', '>=' ,$request->input('starting_date'))->whereDate('created_at', '<=' ,$request->input('ending_date'));
        $q = Picklist_Sale::where('sale_id',$request->sale_id);
        ...
        if($request->input('length') != -1)
            ...
            $limit = $totalData;
        $start = $request->input('start');
        $order = 'picklist_sales.'.$columns[$request->input('order.0.column')];
        $dir = $request->input('order.0.dir');
        if(empty($request->input('search.value'))) {
            $q = Picklist_Sale::
        ...
        else
        {
           ...
           // }
           // else {
                $sales =  $q->select('picklist_sales.*')
                            
                            ->get();

                $totalFiltered = $q->orwhere('picklist_sales.ref_no', 'LIKE', "%{$search}%")
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['key'] = $sale->id;
                $nestedData['date'] = date('M-d-Y', strtotime($sale->created_at));
                $nestedData['reference_no'] = $sale->ref_no;
                $nestedData['name'] = $sale->user_name;
                //$nestedData['customer'] = $sale->customer->name??"".'<br>'.$sale->customer->phone_number??"".'<input type="hidden" class="deposit" value="'.($sale->customer->deposit??"" - $sale->customer->expense??"").'" />'.'<input type="hidden" class="points" value="'.$sale->customer->points??"".'" />';
                $nestedData['products'] = $sale->product_names;
                $nestedData['qty'] = $sale->product_qty;
                $nestedData['picked_by'] = $sale->picked_by;
                //$nestedData['branch'] = $sale->branch->name??"";
                ...
                // }
                // elseif($sale->sale_status == 2){
                //     $nestedData['sale_status'] = '<div class="badge badge-danger">Rejected</div>';
                ...
                //$nestedData['due'] = number_format($sale->grand_total - $returned_amount - $sale->paid_amount, 2);
                $nestedData['options'] = '
                           <a href="'.route('admin.sale.picklist.delete', [$sale->id,$sale->sale_id]).'" class="btn btn-danger btn-link"><i class="fa fa-trash"></i>Delete</a>
                                ';
                        // $nestedData['options'] .= '<li>
                        //     <a href="'.route('sales.edit', $sale->id).'" class="btn btn-link"><i class="dripicons-document-edit"></i> '.trans('file.edit').'</a>
                        //     </li>';
                ...
                //             <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                //                 <li>
                //                 <a href="'.route('admin.report.invoice',$sale->id).'" class="btn btn-link" target="_blank">View
                //                 </a>
                //                 </li>
                //                 <li>
                //                 <a href="'.route('admin.sale.status', [$sale->id,2]).'" class="btn btn-link">Reject
                //                 </a>
                //                 </li>
                //                 <li>
                //                 <a href="'.route('admin.sale.status', [$sale->id,1]).'" class="btn btn-link">Completed
                //                 </a>
                //                 </li>
                //                 </ul>
                //     </div>';
    ...
    public function picklist(Request $request,$id){
        ...
        if($request->input('warehouse_id'))
            ...
            return view('admin.partials.sale.picklist',compact('starting_date', 'ending_date', 'warehouse_id', 'sale_status', 'payment_status','id'));
        

    }

    public function picklistCreate($id){
        $sale = Sale::where('id',$id)->with('user','branch')->get()->toArray();
        $sale_product = Product_Sale::where('sale_id',$id)->with('product')->get();
        //dd($sale_product);
        return view('admin.partials.sale.picklist_create',compact('id','sale','sale_product'));
    ...
    public function picklistSave(Request $request){
        ...
        $item_q = implode(',',$request->item_q);
        //dd($item_name);
        $pic_sale= new Picklist_Sale();
        $pic_sale->product_id = $item_id;
        $pic_sale->product_names = $item_name;
        $pic_sale->product_qty= $item_q;
        $pic_sale->user_name= $request->user_name;
        $pic_sale->sale_id= $request->invoice_number;
        $pic_sale->user_id= $request->user_id;
        $pic_sale->ref_no= $request->ref_no;
        $pic_sale->picked_by= $request->picked_by;
        $pic_sale->pick_date= $request->pick_date;
        
        $pic_sale->save();

        $item_exp = explode(',',$item_id);
        $item_id_a = $request->item_id;
        $item_name_a = $request->item_name;
        $item_q_a = $request->item_q;
 //print_r($item_id[0]);
        foreach ($item_exp as $key => $item_id){
      //  $invoice_picks_items = $this->db->query('select quantity_a from invoices_items Where   `invoice_id`="'.$invoice_number.'" and `id` = "'.$item_id_a[$key].'" ');
        $invoice_picks_items = Product_Sale::where('sale_id',$request->invoice_number)->where('product_id',$item_id_a[$key])->get()->toArray();
        //dd($invoice_picks_items[0]['pic_qty']);
            //print_r($result_picks_is[0]->quantity_a);
            $q=$invoice_picks_items[0]['pic_qty']+$item_q_a[$key];
       $query_inserte = Product_Sale::where('sale_id',$request->invoice_number)->where('product_id',$item_id_a[$key])->update(['pic_qty'=>$q]);
        }
        return 1;
    }


    public function picklistDelete($id,$sale_id) {

        //dd('ok');

        $invoice_picks = Picklist_Sale::where('id',$id)->where('sale_id',$sale_id)->get();

     //dd($invoice_picks[0]['product_id']);

     $idees=explode(',',$invoice_picks[0]['product_id']);
     $idee_q=explode(',',$invoice_picks[0]['product_qty']);
    // print_r($idees);


    foreach ($idees as $key => $item_id){
        
 $invoice_picks_items = Product_Sale::where('id',$item_id)->where('sale_id',$sale_id)->get();

 //dd($invoice_picks_items[0]['pic_qty']);
//             //print_r($result_picks_is[0]->quantity_a);
   $q=$invoice_picks_items[0]['pic_qty']-$idee_q[$key];
        $query_inserte = Product_Sale::where('id',$item_id)->where('sale_id',$sale_id)->update(['pic_qty'=>$q]);   
        }
        //$query = $this->db->query('delete FROM from `pick_list` Where  `id`="'.$id.'"');
       $Picklist_Sale=Picklist_Sale::find($id);
        $Picklist_Sale->delete(); 

      return redirect()->back();

    }

    public function saleStatus(Request $request,$id,$slug){
        //dd($id,$slug);

        Sale::where('id',$id)->update(['sale_status'=>$slug]);
        return redirect()->back();
    ...
    public function store(Request $request)
    {
        ...
        if($lims_sale_data->sale_status == '1')
            return redirect(route('pos.sale.invoice',$lims_sale_data->id))->with('message', $message);
        else
             return redirect(route('pos.sale.pos'))->with('message', $message);
    }
    ...
    public function getProduct()
    {
        ...
        $lims_product_with_variant_warehouse_data = Product::where([
        ...
        //     if($lims_product_data && $lims_product_variant_data ){
    ...
    public function posSale()
    {
            ...
            return view('pos.partials.sale.pos', compact('lims_product_list', 'product_number', 'lims_tax_list', 'lims_brand_list', 'lims_category_list', 'recent_sale', 'recent_draft', 'flag'));
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        //                 ->get();
         $user = Auth::user();
        $lims_product_data = Product::whereHas("pricing",function($query) use($user){
    ...
    public function genInvoice($id)
    {
        ...
        return view('pos.partials.sale.invoice', compact('lims_sale_data', 'lims_product_sale_data', 'lims_payment_data', 'numberInWords'));
    }
