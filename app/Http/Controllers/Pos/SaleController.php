...
class SaleController extends Controller
{
    ...
    public function checkS3Image(Request $request)
    {
//        $imageName = $request->input('imagePath');
...
//
//                if (!$exists) {
//                    // Image doesn't exist in S3, update the database column to null
...
//        }
    ...
    public function checkS3Imageold(Request $request)
    {
        $imageName = $request->input('imagePath');
//        dd($imageName);
        $folderName = 'storage';
    ...
    public function index(Request $request)
    {
        ...
        $promotions = Promotion::whereDate('end_date', '>=', $currentDate)->where('status', 'active')->orderBy('id', 'DESC')->get();
            ...
            return view('pos.partials.sale.index',compact('starting_date', 'ending_date', 'warehouse_id', 'sale_status', 'payment_status','promotions'));
    }
    ...
    public function saleData(Request $request)
    {
        ...
        $q = Sale::where("branch_id",$user->id)->whereDate('created_at', '>=' ,$request->input('starting_date'))->whereDate('created_at', '<=' ,$request->input('ending_date'));
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['date'] = $sale->created_at->format("Y-m-d h:i")??"";
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            //     <li><a href="'.route('pos.sale.invoice', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> '.trans('file.Generate Invoice').'</a></li>
                            //    ';
                ...
                $nestedData['options'] .= 
                    '<li>
                        <button type="button" class="re-order btn btn-link" data-id = "'.$sale->id.'"><i class="fa fa-truck"></i> Re-order</button>
                    </li>
                    <li>
                                <a href="'.route('pos.report.invoice',$sale->id).'" class="btn btn-link" target="_blank"><i class="fa fa-eye"></i>View
                                </a>
                                </li>
                                <li>
                                <a href="'.route('pos.sale.status', [$sale->id,2]).'" class="btn btn-link"><i class="fa fa-close"></i>Reject
                                </a>
                                </li>
                                <li>
                                <a href="'.route('pos.sale.status', [$sale->id,1]).'" class="btn btn-link"><i class="fa fa-check"></i>Completed
                                </a>
                                </li>
                    </ul>
                     </div>';
    ...
    public function getProduct()
    {
        ...
        {
            $price = $product->pricing()->where("user_id",$user->user_id)->first();
            ...
            $product_batch_id[] = 0;
            $expired_date[] = 0;
            $product_price[] = $price?$price->sale_price:0;
            //if($product_warehouse->is_embeded)
...
//        }])->where("user_id",$user->user_id)->count();
        ...
        return view('pos.partials.pos.pos', compact('products', 'categories', 'brands', 'user','promotions'));
    }
    ...
    public function getProductByFilter($category_id, $brand_id)
    {
        ...
        $lims_product_list = ProductPricingManagement::select("product_id", "sale_price")->whereHas("product",function($query){
            $query->where(['is_active'=>true]);
        })->with(["product"=>function($query){
            $query->select("id", "name", "code",'brand_id' ,"image","ms_id");
        }])->where("user_id",$user->user_id)->where("cate",$category_id)->get();
        }
        ...
        elseif(($category_id == 0) && ($brand_id != 0)){
            ...
            $lims_product_list = Product::where("user_id",$user->user_id)->where('is_active', true)->get();
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        $product[] = $lims_product_data->id;
        $product[] = false;//$lims_product_data->is_batch;
        $product[] = false;//$lims_product_data->is_imei;
        $product[] = $lims_product_data->is_variant??false;
        $product[] = $qty;
        return $product;
    ...
    public function addPayment(Request $request)
    {
        ...
        elseif ($paying_method == 'Paypal') {
            ...
            return redirect('admin/'.$response['paypal_link']);
        }
    ...
    public function deletePayment(Request $request)
    {
        ...
        return redirect('admin/sales')->with('not_permitted', 'Payment deleted successfully');
    }
...
//        }])->where("user_id",$user->user_id)
//            ->paginate(500);
                $products = ProductPricingManagement::select("product_id", "price", 'p_price', "is_favorite", "cate")
            ->with(["product" => function ($query) {
                $query->select("id", "name", "code", "pack_size", "image", "category_id", "brand_id", 'tax_id', 'tax_method')->with("tax");
            }])
            ->where("user_id", $user->user_id)
    ...
    public function posSaleSubmit(Request $request, MSDynamic $api){
        ...
        // dd($user);
        // $warehouse = Warehouse::where('id', $branch->warehouse)->first();
        $ids = [];

        
        $sale = Sale::create($data);
        // $collectData['HeaderNo'] = "SL-ORD-0".$sale->id;
        // $collectData['DocumentType'] = "Order";
        // $collectData['LocationCode'] = $warehouse->name;//$order->warehouse->name;
        // $collectData['ResponsibilityCenter'] = $warehouse->name;// $order->user->name;
        ...
        // $collectData['postingno'] = "SL-ORD-0-".$sale->id;
        // $collectData['posnotes'] = null;
        // $collectData['ExtDocumentNo'] = "SL-ORD-0".$sale->id;
        // $collectData['PaymentTerms'] = "";
        // $collectData['Source'] = "POS";
        if($request->items){
...
//                $product_sale['tax'] = $item['tax'];
                $product_sale['tax'] = $item['product_tax'] * $item['qty'];
//                $product_sale['total'] = ($item['price']+$item['product_tax']) * $item['qty'];
                $product_sale['total'] = $item['price'] * $item['qty'];
                // dd($item);
        ...
        if($sale_datas){
            foreach($sale_datas as $sale_data){
                ...
                $data[] = [
                    ...
                    'image'=>'/storage/thumbnail/'.$image[0]??"",
