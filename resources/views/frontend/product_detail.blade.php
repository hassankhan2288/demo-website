...
@section('main-content')
    <style>
        .free-products-table {
            ...
            border-spacing: 0;
            /* Set border-spacing to 0 to remove spacing between cells */
            width: 100%;
            border: 1px solid #ccc;
            /* Add a border around the entire table */
        }

        .free-products-table th,
        .free-products-table td {
            padding: 8px;
            text-align: left;
        }
    ...
    <main>
            ...
            <div class="!container px-6 mx-auto">
                <div class="grid lg:grid-cols-12 grid-cols-1 !gap-8">
                    <div class="xl:col-span-4 lg:col-span-5">
                        <!-- Product Slider -->
                        <div
                            class="!border !border-[#ce1212]/10 text-center rounded-[10px] p-[15px] hover:!border-[#ce1212] transition duration-300">
                                        ...
                                        <div class="swiper-slide min-h-[400px] flex flex-column justify-center">
                                            <div class=" flex justify-center items-center">
                                                <div data-thumb="{{ asset('storage/' . $data) }}" rel="adjustX:10, adjustY:">
                                                    ...
                                                    @endphp
                                                    <img src="{{ image_url('storage/' . $data) }}"
                        ...
                        <div class="thumbnail-swiper swiper w-full mt-3">
                            <div class="swiper-wrapper">
                                @foreach ($photo as $data)
                                    <div
                                        class="swiper-slide !border !border-[#ce1212]/10 text-center rounded-[10px] p-[10px] hover:!border-[#ce1212] transition duration-300">
                    ...
                    <div class="xl:col-span-8 lg:col-span-7">
                        ...
                        <h5 class="sm:text-[1rem] text-[10px] font-medium mb-[10px]"><strong>Product Code :</strong>
                            {{ $product_detail->ms_id }}</h5>
                        <h5 class="sm:text-[1rem] text-[10px] font-medium mb-[10px]"><strong>Pack Size :</strong>
                            {{ $product_detail->pack_size }}</h5>
                            ...
                            <div class="d-flex custom_design_hm productdetailsprice">
                                <div class="collectionPrice">
                                    <span class="d-flex">
										...
										@if ($product_detail->Fullprice && $product_detail->Fullprice->price != 0)
											<div class="caseprice">
												<strong>Rs{{ number_format($product_detail->Fullprice->price, 2) }}</strong>
											</div>
										@endif


                                        @if ($product_detail->Fullprice && $product_detail->Fullprice->p_price != 0)
                                            <div class="singleprice">
                                                <strong>Rs{{ number_format($product_detail->Fullprice->p_price, 2) }}</strong>
                                            </div>
                                        @endif
                                    </span>
                                </div>
                                ...
                                <div class="deliveryPrice">
                                    <span class="d-flex">
                                        <h3>Delivery</h3>
                                        @if ($product_detail->Fullprice && $product_detail->Fullprice->delivery_single != 0)
                                            <div class="singleprice">
                                        ...
                                        @endif
                                        @if ($product_detail->Fullprice && $product_detail->Fullprice->delivery_pack != 0)
                                            <div class="caseprice">
                            ...
                            <div class="container btn_container">
                                ...
                                @if (\Auth::guard('customer')->user()->checkout_preference == 'delivery')
                                    @if ($product_detail->Fullprice)
                                        @if ($product_detail->Fullprice->delivery_pack > 0 && $product_detail->Fullprice->delivery_single <= 0)
                                            <button class="btn price_selector selected_price" data-type="CASE">Pack Price:
                                                £{{ $product_detail->Fullprice->delivery_pack }}</button>
                                        @endif
                                        @if ($product_detail->Fullprice->delivery_single > 0 && $product_detail->Fullprice->delivery_pack <= 0)
                                            <button class="btn price_selector selected_price" data-type="SINGLE">Single
                                                Price: £{{ $product_detail->Fullprice->delivery_single }}</button>
                                        @endif
                                        @if ($product_detail->Fullprice->delivery_single > 0 && $product_detail->Fullprice->delivery_pack > 0)
                                            <button class="btn price_selector selected_price" data-type="CASE">Pack Price:
                                                £{{ $product_detail->Fullprice->delivery_pack }}</button>
                                            <button class="btn price_selector" data-type="SINGLE">Single Price:
                                                £{{ $product_detail->Fullprice->delivery_single }}</button>
                                        @endif
                                    @endif
                                @endif
                                ...
                                @if (\Auth::guard('customer')->user()->checkout_preference == 'pickup')
                                    @if ($product_detail->Fullprice)
                                        @if ($product_detail->Fullprice->p_price > 0 && $product_detail->Fullprice->price <= 0)
                                            <button class="btn price_selector selected_price" data-type="CASE">Pack Price:
                                                £{{ $product_detail->Fullprice->p_price }}</button>
                                        @endif
                                        @if ($product_detail->Fullprice->price > 0 && $product_detail->Fullprice->p_price <= 0)
                                            <button class="btn price_selector selected_price" data-type="SINGLE">Single
                                                Price: £{{ $product_detail->Fullprice->price }}</button>
                                        @endif
                                        @if ($product_detail->Fullprice->price > 0 && $product_detail->Fullprice->p_price > 0)
                                            <button class="btn price_selector selected_price" data-type="CASE">Pack Price:
                                                £{{ $product_detail->Fullprice->p_price }}</button>
                                            <button class="btn price_selector" data-type="SINGLE">Single Price:
                                                £{{ $product_detail->Fullprice->price }}</button>
                                        @endif
                                    @endif
                                @endif
						...
						@endif --}}
							...
							@endif --}}
                                    <input type="hidden" name="preference"
                                        value="{{ \Auth::guard('customer')->user()->checkout_preference }}">
                                    <input type="hidden" name="slug" value="{{ $product_detail->slug }}">
                                    <input type="hidden" name="product_id" value="{{ $product_detail->id }}">
                                    <div class="qty-input">
                                        <button class="qty-count qty-count--minus" data-action="minus"
                                            type="button">-</button>
                                        <input class="product-qty" type="number" name="quant[1]"
                                            class="w-[70px] p-[8px_5px] rounded-[5px] !border border-[#ce1212]/10"
                                            min="1" max="10" data-min="1" data-max="10" value="1"
                                            id="quantity">
                                ...
                                <div class="flex items-center flex-wrap gap-3 mt-4">
                                    @if ($stock == 0 and $warehouse_id != 0)
                                        ...
                                        <button type="submit"
                                            class="bg-[#ce1212] text-white p-[11px_30px] rounded-[50px] text-[14px] font-bold disabled:opacity-70"
                                            disabled>Add to cart</button>
                                        <a href="{{ route('add-to-wishlist', $product_detail->slug) }}"
                                            class="bg-[#ce1212] text-white p-[11px_30px] rounded-[50px] text-[14px] font-bold opacity-70">
                                    ...
                                    @else
                                        ...
                                        @if (\Auth::guard('customer')->user()->checkout_preference == 'delivery')
                                            @if ($product_detail->Fullprice)
                                                @if ($product_detail->Fullprice->delivery_pack > 0 || $product_detail->Fullprice->delivery_single > 0)
                                                    <button type="submit"
                                                        class="bg-[#ce1212] text-white p-[11px_30px] rounded-[50px] text-[14px] font-bold">Add
                                                        to cart</button>
                                                    <a href="{{ route('add-to-wishlist', $product_detail->slug) }}"
                                                        class="bg-[#ce1212] text-white p-[11px_30px] rounded-[50px] text-[14px] font-bold"><i
                                                ...
                                                @foreach ($free_pro as $pro)
                                                    <tr>
                                                        <td class="product-image-cell">
                                                            <img src="{{ asset('path_to_your_product_image_directory/' . $pro->image) }}"
        ...
        <section class="pt-[40px] pb-[110px]">
            <div class="!container !px-6 mx-auto relative">
                ...
                <div
                    class="swiper-button-prev single-button-prev after:content-[''] w-[40px] h-[40px] rounded-full bg-[#ce1212]/10 transition duration-500 top-[22px] right-12 left-[unset]">
                    <i class="fa fa-angle-left text-black text-[22px]" aria-hidden="true"></i>
                </div>
                <div
                    class="swiper-button-next single-button-next after:content-[''] w-[40px] h-[40px] rounded-full bg-[#ce1212]/10 transition duration-500 top-[22px] right-0">
